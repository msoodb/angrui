...
export default {
  ...
  methods: {
    ...
    getItem(){
      ...
      this.$axios.get(baseurl() + '/tags_contents/' + id, config )
    ...
    addItem(){
      ...
      this.$axios.post(baseurl() + '/tags_contents', data_request, config )
        .then(function (response) {
          if(response.status == 200){
    ...
    updateItem(){
      ...
      var data_request = JSON.stringify(self.form);
      this.$axios.put(baseurl() + '/tags_contents/' + id, data_request, config )
        .then(function (response) {
          if(response.status == 200){
            let currentMsg =  self.$message  ({
