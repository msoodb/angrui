  ...
  <el-tabs type="border-card">
    <el-tab-pane label="General">
      <el-form ref="form" :model="form" :rules="rules" label-width="140px" inline-message>
        <el-row :gutter="20">
          <el-col :span="16">
            <el-form-item label="member" prop="member_lookup">
              <au-member-lookup handler="members" :id="form.member" @select="MemberLookupSelect"></au-member-lookup>
            </el-form-item>
            <el-form-item label="service" prop="service">
              <au-lookup handler="services" :id="form.service" @select="ServiceLookupSelect"></au-lookup>
            </el-form-item>
            <el-form-item label="last_login">
              <el-date-picker v-model="form.last_login" type="date" placeholder="Pick a day"></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="8">
...
import AUMemberLookup from '../../components/AU-MemberLookup'
...
export default {
  ...
  components: {
    'au-lookup' : AULookup,
    'au-member-lookup' : AUMemberLookup
  },
  // mounted(){
  //   if(this.record_id != "-1"){
  //     this.getItem();
  ...
  methods: {
    ...
    getItem(){
      ...
      this.$axios.get(baseurl() + '/subscriptions/' + id, config )
        .then(function (response) {
          if(response.status == 200){
            self.form.id = response.data.id;
            self.form.member = response.data.member;
            self.form.service = response.data.service;
            self.form.last_login = response.data.last_login;
    ...
    addItem(){
      ...
      this.$axios.post(baseurl() + '/subscriptions', data_request, config )
        .then(function (response) {
          if(response.status == 200){
    ...
    updateItem(){
      ...
      if(!self.form.details || self.form.details==''){self.form.details = "{}"}
      var data_request = JSON.stringify(self.form);
      this.$axios.put(baseurl() + '/subscriptions/' + id, data_request, config )
        .then(function (response) {
          if(response.status == 200){
            let currentMsg =  self.$message  ({
