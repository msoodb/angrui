        ...
        <el-row :gutter="20">
          <el-col :span="16">
            ...
            <el-form-item label="content provider" prop="content_provider">
              <au-lookup handler="content_providers" :id="form.content_provider" @select="SecurityRoleLookupSelect"></au-lookup>
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getItem(){
      ...
      this.$axios.get(baseurl() + '/users_content_providers/' + id, config )
    ...
    addItem(){
      ...
      this.$axios.post(baseurl() + '/users_content_providers', data_request, config )
        .then(function (response) {
          if(response.status == 200){
    ...
    updateItem(){
      ...
      var data_request = JSON.stringify(self.form);
      this.$axios.put(baseurl() + '/users_content_providers/' + id, data_request, config )
        .then(function (response) {
          if(response.status == 200){
            let currentMsg =  self.$message  ({
